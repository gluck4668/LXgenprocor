...
LXgenprocor <- function(gene_data,protein_data,FC,species){
  ...
  if(is.null(FC))
    ...
    RNA <- dplyr::filter(RNA,pvalue<0.05 & abs(log2FC)>=FC)
  head(RNA)
  ...
  if(is.null(FC))
    ...
    protein <- dplyr::filter(protein,pvalue<0.05 & abs(log2FC)>=FC)
  ...
  titile_text <- paste('Genes/Proteins Correlation Analysis')
  ...
  ggsave("analysis result/Genes-Proteins Correlation Analysis.png",
